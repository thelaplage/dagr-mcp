"""LOCAL-DEMO-ONLY zero-argument DAGR adapter factory for WAVE100 capture.

.. warning::

   **LOCAL DEMO ONLY. EXPLICITLY NON-PRODUCTION.** This module encodes a single,
   owner-frozen, deliberately narrow demo policy for
   ``WAVE100-GOVERNED-EXEC1`` — the governed capture of the frozen
   DEMO-PARTIAL-FRONTIER1 batch0 (100 locators). It is not a production policy
   pack, is not wired into any released runtime path, and must never be used to
   govern a real institutional trust boundary.

Canonical selector (what a caller passes to acquisition's ``load_dagr_adapter``)::

    dagr_mcp_local_demo.wave100_acquisition:build_adapter

This is a SECOND, distinct local-demo factory alongside
``dagr_mcp_local_demo.counterpedia_acquisition`` (the LIVE-AUTHORING-ACCEPT0
factory, which admits ONLY ``acquisition.process_held_capture``). That factory is
frozen and is neither modified nor widened here; using it for WAVE100 would
correctly REFUSE ``acquisition.capture_url``. This module provides the narrow
WAVE100 policy instead.
...
* **scope** — ``WAVE100-GOVERNED-EXEC1`` local demo only; NON-PRODUCTION.
...
* **actor** — FIXED ``actor:counterpedia-local-demo:wave100-operator``. It is
...
* **ids** — ``runtime_instance_id``, ``boundary_id``, ``policy_pack_id`` and
  ``policy_pack_version`` are fixed to the frozen WAVE100 local-demo values below.
...
The batch/domain scope (the exact 100 rows, the 3 domains) is NOT re-encoded
here: it is proven and enforced by the frozen WAVE100 runner in
counterpedia-acquisition (``wave100_governed_exec``), and the SdkBindingConfig
interface exposes no canonical per-argument/domain constraint seam to duplicate
it into. This factory governs the *tool* (``acquisition.capture_url`` admit /
everything-else refuse); the runner governs *which locators* are driven.

Nothing in this module removes acquisition's requirement that an operator supply
a factory; it merely provides one acceptable, narrow, WAVE100 local-demo factory.
"""
...
# --------------------------------------------------------------------------- #
# Frozen WAVE100 local-demo policy constants (do NOT broaden)                 #
# --------------------------------------------------------------------------- #
...
#: content.
FIXED_ACTOR_REF: str = "actor:counterpedia-local-demo:wave100-operator"

#: Refusal reason code emitted for every out-of-scope (non-allowed) tool.
REFUSAL_REASON_CODE: str = "local_demo_tool_not_allowed"

RUNTIME_INSTANCE_ID: str = "counterpedia-local-demo:wave100-governed-exec1"
BOUNDARY_ID: str = "counterpedia-acquisition-mcp:wave100-governed-exec1"
POLICY_PACK_ID: str = "policy:counterpedia-local-demo:wave100-governed-exec1"
...
_ISSUER_ID: str = "urn:counterpedia-local-demo:nonprod-ephemeral-issuer:wave100"
...
#: WAVE100-specific variable keeps this evidence trail disjoint from the
#: LIVE-AUTHORING-ACCEPT0 factory's.
EVIDENCE_DIR_ENV: str = "COUNTERPEDIA_LOCAL_DEMO_WAVE100_EVIDENCE_DIR"
...
def _resolve_evidence_dir() -> Path:
    ...
    Requires ``COUNTERPEDIA_LOCAL_DEMO_WAVE100_EVIDENCE_DIR`` to name an existing,
...
def build_adapter() -> SdkLifecycleAdapter:
    ...
    (``COUNTERPEDIA_LOCAL_DEMO_WAVE100_EVIDENCE_DIR``) is missing or invalid. On
    ...
    WAVE100 actor/ids, and the evidence env:
