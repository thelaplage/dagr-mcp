"""Local copy of tests/receipt_verification.py's jsonschema + Ed25519 verifier.

Copied (not imported) so packages/dagr-mcp-core's test suite can be run in a
completely standalone checkout of this package (e.g. from a built sdist/wheel)
without depending on the root repo's ``tests/`` tree.
"""
