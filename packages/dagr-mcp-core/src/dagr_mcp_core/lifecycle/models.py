...
Fork provenance: one-way extraction fork of ``dagr_mcp_lifecycle/models.py`` as
...
:mod:`dagr_mcp_core.lifecycle.core`. Every value here is an ordinary frozen
...
The vocabulary is imported from :mod:`dagr_mcp_core.lifecycle.contract` so the
core and the contract are a single source of truth.
"""
...
from dagr_mcp_core.lifecycle.contract import (
