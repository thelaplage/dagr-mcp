...
Fork provenance: one-way extraction fork of ``dagr_mcp_lifecycle/core.py`` as
of the commit recorded in
``packages/dagr-mcp-core/EXTRACTION_MANIFEST.json``. Copied verbatim (no
content delta) except this module docstring and the internal import path
(``dagr_mcp_lifecycle`` -> ``dagr_mcp_core.lifecycle``). See
``docs/CORE_EXTRACTION_FORK.md`` for the fork and maintenance policy.

**What the core is.** A set of pure transition/planning functions over the
explicit typed values in :mod:`dagr_mcp_core.lifecycle.models`. Given a
resolved admission decision and an already-classified execution observation, it
returns a :class:`~dagr_mcp_core.lifecycle.models.LifecyclePlan` describing
which records exist, what neutral facts each carries, and which
responsibilities belong to the adapter.
...
the neutral :mod:`dagr_mcp_core.lifecycle.contract` /
:mod:`dagr_mcp_core.lifecycle.models` vocabulary, so the core stays a single
source of truth with its own contract.
"""

from __future__ import annotations

from dagr_mcp_core.lifecycle import contract
...
# bound by the binding-specific mask; the core owns only which families apply.
...
# families each outcome record adds. Read straight from the contract so a
...
def _admission_before_execution(request: AdmissionRequest) -> bool:
    ...
    it only when the binding is configured to (the default).
    """
...
def plan_admission(request: AdmissionRequest) -> AdmissionPlan:
    ...
    * **refused** — a single terminal admission record carrying the neutral
      ...
      ground is preserved as-is and never repaired.
    ...
    * **admitted** — for write/destructive, or a read configured to observe
    ...
    if disposition == "deferred":
        ...
        if request.review_object_created is False:
            ...
            # check). The core surfaces the resolved refusal explicitly.
            record = AdmissionRecordIntent(
                disposition="refused",
...
def _outcome_record(observation: ExecutionObservation) -> OutcomeRecordIntent:
    ...
    # TimeoutError. It is planned, not unsupported.
...
# unsupported. This lets a test prove the core implements-or-declares-unsupported
# for every contract token.
