...
Fork provenance: one-way extraction fork of ``dagr_mcp_lifecycle/contract.py``
as of the commit recorded in
...
in ``dagr_mcp_lifecycle.binding_mask`` (legacy tree only — not forked here; see
the fork-content rules in ``docs/CORE_EXTRACTION_FORK.md``).

Every name here is a *contract token*, not an implementation. The tokens are
frozen so a drift test can pin them, exactly as the Sprint A1 behavioral freeze
pins the binding's own observable surface.
