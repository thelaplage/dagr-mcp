"""Hostile-gate tests for the LOCAL-DEMO WAVE100 acquisition adapter factory.

Correctness tests for ``dagr_mcp_local_demo.wave100_acquisition:build_adapter``
and the narrow, owner-frozen WAVE100 local-demo policy it encodes. They assert
...
from dagr_mcp_local_demo import wave100_acquisition as factory
from dagr_mcp_local_demo.wave100_acquisition import (
    ALLOWED_TOOL,
    EVIDENCE_DIR_ENV,
    FIXED_ACTOR_REF,
    REFUSAL_REASON_CODE,
...
async def test_g7_capture_url_admitted(evidence_dir: Path) -> None:
    ...
    assert a.get("actor_ref") == FIXED_ACTOR_REF == "actor:counterpedia-local-demo:wave100-operator"
    assert a["policy_pack_id"] == "policy:counterpedia-local-demo:wave100-governed-exec1"
    assert a["policy_pack_version"] == "0.1"
    assert a["boundary_id"] == "counterpedia-acquisition-mcp:wave100-governed-exec1"
    assert a["runtime_instance_id"] == "counterpedia-local-demo:wave100-governed-exec1"
...
async def test_process_held_capture_specifically_refused(evidence_dir: Path) -> None:
    # The two demo factories are disjoint: WAVE100 refuses the LIVE-AUTHORING tool.
    adapter = build_adapter()
    delegate = _RecordingDelegate()
    with pytest.raises(ToolRefused):
