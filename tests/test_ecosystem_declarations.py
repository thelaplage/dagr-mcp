...
def _find_kit_schemas() -> Path | None:
    """Locate a pinned sibling arcs-ecosystem-kit checkout, offline, without adding a dependency.

    Checked, in order: an explicit override (for worktree checkouts, where
    ``ROOT.parent`` is not the sibling directory that holds other cloned
    repositories), then the conventional sibling-of-this-repository layout.
    """
    override = os.environ.get("ARCS_ECOSYSTEM_KIT_SCHEMAS")
    if override:
        return Path(override)
    return ROOT.parent / "arcs-ecosystem-kit" / "schemas"


KIT_SCHEMAS = _find_kit_schemas()

requires_ecosystem_kit = pytest.mark.skipif(
    KIT_SCHEMAS is None or not KIT_SCHEMAS.exists(),
    reason=(
        "arcs-ecosystem-kit schemas not found beside dagr-mcp and "
        "ARCS_ECOSYSTEM_KIT_SCHEMAS is not set; schema validation skipped"
    ),
)
